...
object OrgmodeReplacePatternGenerator {
    ...
    @JvmField
    val PREFIX_ATX_HEADING: Pattern = Pattern.compile("^(\\s{0,3})(\\*+\\s)")
    @JvmField
    val PREFIX_CHECKED_LIST: Pattern = Pattern.compile("^(\\s*)((-|\\+)\\s\\[(X)]\\s)")
    @JvmField
    val PREFIX_CHECKBOX_LIST: Pattern = Pattern.compile("^(\\s*)(([-+]\\s\\[)[\\sX](]\\s))")
    @JvmField
