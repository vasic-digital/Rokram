...
import digital.vasic.yole.frontend.AttachLinkOrFileDialog;
...
public class DocumentShareIntoFragment extends YoleBaseFragment {
    ...
    public static DocumentShareIntoFragment newInstance(final Intent intent, final Context context) {
        ...
        f.attachment = YoleContextUtils.getIntentFile(intent, context);
        return f;
    }
    ...
    public static class ShareIntoImportOptionsFragment extends GsPreferenceFragmentBase<AppSettings> {
        ...
        private void selectOrCreateDestination(final @Nullable File startFolder) {
            ...
            }, getParentFragmentManager(), getActivity(), YoleFileBrowserFactory.IsMimeText);
        }
        ...
        private void searchForTarget() {
            ...
            YoleDialogFactory.showNotebookFilterDialog(getActivity(), null, filter, (file, isLong) -> {
                if (isLong) {
        ...
        public Boolean onPreferenceClicked(Preference preference, String key, int keyId) {
            final Activity activity = getActivity();
            final String text = _editor.getText().toString();
            final YoleContextUtils shu = new YoleContextUtils(activity);
            String tmps;
