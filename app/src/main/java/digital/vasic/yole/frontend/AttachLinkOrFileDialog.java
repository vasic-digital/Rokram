...
import digital.vasic.yole.format.wikitext.WikitextLinkResolver;
...
public class AttachLinkOrFileDialog {
    ...
    private static GsCallback.b2<Context, File> getFilterForAction(final InsertType action) {
        switch (action) {
            ...
            case IMAGE_DIALOG:
            case IMAGE_BROWSE: {
                return YoleFileBrowserFactory.IsMimeImage;
            }
            case AUDIO_RECORDING:
            case AUDIO_DIALOG:
            case AUDIO_BROWSE: {
                return YoleFileBrowserFactory.IsMimeAudio;
            }
            case LINK_DIALOG:
            case LINK_BROWSE:
            case LINK_SPECIAL:
    ...
    private static void fetchAndInsertItem(
        ...
        final YoleContextUtils cu = new YoleContextUtils(activity);
        ...
        switch (action) {
            ...
            case AUDIO_BROWSE: {
                if (activity instanceof AppCompatActivity && nameEdit != null && pathEdit != null) {
                    ...
                    YoleFileBrowserFactory.showFileDialog(fsListener, f, activity, getFilterForAction(action));
                }
                break;
            }
            case LINK_SPECIAL: {
                YoleDialogFactory.showSelectSpecialFileDialog(activity, setFields);
                break;
            }
            case LINK_SEARCH: {
                YoleDialogFactory.showNotebookFilterDialog(activity, null, null, (file, l) -> {
                    setFields.callback(file);
                });
            }
            case LINK_DIALOG:
