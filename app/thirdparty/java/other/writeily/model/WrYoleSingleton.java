...
public class WrYoleSingleton {

    private static WrYoleSingleton markorSingletonInstance = null;
    private static File notesLastDirectory = null;

    public static WrYoleSingleton getInstance() {
        if (markorSingletonInstance == null) {
            markorSingletonInstance = new WrYoleSingleton();
        }
    ...
    public void setNotesLastDirectory(File notesLastDirectory) {
        WrYoleSingleton.notesLastDirectory = notesLastDirectory;
    }
        ...
        while (!files.empty()) {
            ...
            if (dest.exists()) {
                ...
                if (resolution == ConflictResolution.KEEP_BOTH || (!isMove && file.equals(dest))) {
                    ...
                    files.push(file);
                    YoleDialogFactory.showCopyMoveConflictDialog(activity, file.getName(), destDir.getName(), files.size() > 1, (option) -> {
